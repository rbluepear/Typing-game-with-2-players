...
private:
	const float SPEED_RUN = 100.0f; //每秒移动100像素距离
...
public:
	...
	void on_update(float delta) {
		...
		if ((pos_target - position).length() <= (velocity * delta).length())
			position = pos_target; // 可越过目标位置，则定位置于目标处
		else
		...
		if (velocity.approx(Vector2(0, 0))) {
			...
			switch (facing)
			{
			case Player::Facing::Up:	current_anim = &anim_idle_up;
				break;
			case Player::Facing::Down:	current_anim = &anim_idle_down;
				break;
			case Player::Facing::Left:	current_anim = &anim_idle_left;
				break;
			case Player::Facing::Right:	current_anim = &anim_idle_right;
				break;
			}
		}
		else {
			...
			switch (facing)
			{
			case Player::Facing::Up:	current_anim = &anim_run_up;
				break;
			case Player::Facing::Down:	current_anim = &anim_run_down;
				break;
			case Player::Facing::Left:	current_anim = &anim_run_left;
				break;
			case Player::Facing::Right:	current_anim = &anim_run_right;
				break;
			}
		}
